...
public class CourseRegistrationResource {
    ...
    /**
     * Simply gets a list of all course registrations. only ADMIN_ROLE user can do this.
     * @return
     */
    @GET
     ...
     * @param profId The professor Id passed as a parameter from POST request
     * @param newRegistration Course Registration details to be used when creating new course registration
     * @return the response
     */
    @POST
    @RolesAllowed({ADMIN_ROLE})
    ...
    public Response addCourseRegistration(@PathParam("courseId") int courseId ,
		...
		if (regStudent == null || regCourse == null) {
			response = Response.status(Status.NOT_MODIFIED).build();
			 ...
			 CourseRegistration persistedReg = service.persistRegistration(newRegistration, regStudent, regCourse, regProf);
    ...
    /**
     * Used to delete a CourseRegistration with primary key composed of both parameter
     * values. Uses CourseRegistrationService method deletRegistrationById
...
//    	response = Response.ok(deletedRegistration).build();
    ...
    /**
     * Resource for seeing a list of courses for a student with provided ID. If user is a USER_ROLE then the 
     * id of the user must match the id of the student for which a course list is requested. Other wise if
     * user is ADMIN_ROLE then they are able to look at any students course list.
     * @param id
     * @return
     */
    @GET
